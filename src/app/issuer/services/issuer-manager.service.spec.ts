...
xdescribe("IssuerManager", () => {
		...
		(
			...
			mockBackend.connections.subscribe((connection: MockConnection) => {
				if (connection.request.url !== "/v1/issuer/issuers") {
					fail(
						"Only issuer list calls are allowed. No other API calls should occur when counting issuer entities."
					);
				}
			});
		...
		(
			...
			return Promise.all([
				...
				verifyEntitySetWhenLoaded(issuerManager.issuersList, [existingIssuer])
					...
					.then(() =>
						verifyManagedEntitySet(issuerManager.issuersList, [
							newIssuer,
							existingIssuer
						])
					)
	...
	it("should handle adding staff members", inject(
		...
		(
			...
			return Promise.all([
				...
				verifyEntitySetWhenLoaded(issuerManager.issuersList, [existingIssuer])
					.then(issuersList =>
						issuersList.entities[0].addStaffMember("staff", "new@user.com")
					)
					.then(issuer => {
						expect(
							issuer.staff.entityForApiEntity(newStaffMember).apiModel
						).toEqual(newStaffMember);
					})
			]);
		}
	));
	...
	it("should handle modifying staff members", inject(
		...
		(
			...
			return Promise.all([
				...
				verifyEntitySetWhenLoaded(issuerManager.issuersList, [existingIssuer])
					.then(issuersList => {
						const member = issuersList.entities[0].staff.entityForApiEntity(
							modifiedStaffMember
						);
	...
	it("should handle deleting staff members", inject(
		...
		(
			...
			const existingIssuer = apiIssuer1;
			const staffMemberToRemove: ApiIssuerStaff = existingIssuer.staff.find(
				s => s.role === "staff"
			);
			...
			return Promise.all([
				...
				verifyEntitySetWhenLoaded(issuerManager.issuersList, [existingIssuer])
					.then(issuersList =>
						issuersList.entities[0].staff
							.entityForApiEntity(staffMemberToRemove)
							.remove()
					)
					.then(issuer => {
						expect(issuer.staff.entities.map(s => s.apiModel)).not.toContain(
							staffMemberToRemove
						);
					})
			]);
