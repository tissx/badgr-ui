...
} from "../../common/model/managed-entity-set.spec";
import {
	apiBadgeClass1,
	apiBadgeClass2,
	apiBadgeClass3
} from "../models/badgeclass.model.spec";
...
xdescribe("badgeManager", () => {
	...
	it("should retrieve all badge classes", inject(
		[BadgeClassManager, SessionService, MockBackend],
		(
			badgeManager: BadgeClassManager,
	...
	it("should retrieve badges on subscription of allBadges$", inject(
		[BadgeClassManager, SessionService, MockBackend],
		(
			badgeManager: BadgeClassManager,
			loginService: SessionService,
	...
	it("should retrieve badges on subscription of badgesByIssuerUrl$", inject(
		[BadgeClassManager, SessionService, MockBackend],
		(
			badgeManager: BadgeClassManager,
			loginService: SessionService,
			...
			return Promise.all([
				expectAllBadgesRequest(mockBackend),
				badgeManager.badgesByIssuerUrl$
					.pipe(first())
					.toPromise()
					.then(() =>
						verifyManagedEntitySet(badgeManager.badgesList, allApiBadgesClasses)
	...
	it("should remove a badge class", inject(
		[BadgeClassManager, SessionService, MockBackend],
		(
			badgeManager: BadgeClassManager,
			loginService: SessionService,
			...
			return Promise.all([
				...
				expectRequestAndRespondWith(
					...
					`/v1/issuer/issuers/${BadgeClass.issuerSlugForApiBadge(
						badgeToRemove
					)}/badges/${badgeToRemove.slug}`,
					`Badge ${badgeToRemove.slug} has been deleted.`
				),
				verifyEntitySetWhenLoaded(badgeManager.badgesList, [
					badgeToRemove,
					badgeToKeep
				])
					.then(badgeList =>
						badgeManager.removeBadgeClass(
							badgeList.entityForApiEntity(badgeToRemove)
						)
					)
					.then(() =>
						verifyManagedEntitySet(badgeManager.badgesList, [badgeToKeep])
					)
			]);
		}
	));

	it("should add a new badge class", inject(
		[BadgeClassManager, SessionService, MockBackend],
		(
			badgeManager: BadgeClassManager,
			loginService: SessionService,
			...
			return Promise.all([
				...
				verifyEntitySetWhenLoaded(badgeManager.badgesList, [existingBadge])
					.then(badgeList =>
						badgeManager.createBadgeClass(
							BadgeClass.issuerSlugForApiBadge(newBadge),
							newBadge
						)
					)
					.then(() =>
						verifyManagedEntitySet(badgeManager.badgesList, [
							newBadge,
							existingBadge
						])
					)
...
export function testBadgeClassRefForSlugs(
	issuerSlug: string,
