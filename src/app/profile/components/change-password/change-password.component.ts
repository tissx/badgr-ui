...
export class ChangePasswordComponent extends BaseRoutableComponent {
	...
	constructor(
		...
		title.setTitle(
			`Change Password - ${this.configService.theme["serviceName"] || "Badgr"}`
		);
	...
	submitChange() {
		...
		this.profile
			...
			.then(
				() => {
					this._messageService.reportMajorSuccess(
					...
					if (err.message && this.isJson(err.message)) {
						...
						for (const key in errors) {
							if (errors.hasOwnProperty(key)) {
								...
								const c = this.changePasswordForm.controls[
									key
								].rawControl.valueChanges.subscribe(val => {
