...
@Component({
	...
	template: `
		<div class="badgecard">
			<div
				class="badgecard-x-status badgestatus badgestatus-{{
					mostRelevantStatus
				}}"
			...
			<div class="badgecard-x-body">
				...
				<a
					*ngIf="badgeSlug"
					class="badgecard-x-title"
					[routerLink]="['../earned-badge', badgeSlug]"
					>{{ badgeTitle }}</a
				>
				<a *ngIf="publicUrl" class="badgecard-x-title" [href]="publicUrl">{{
					badgeTitle
				}}</a>
				<div class="badgecard-x-issuer">{{ issuerTitle }}</div>
				<p
					class="badgecard-x-desc"
					[truncatedText]="badgeDescription"
					[maxLength]="100"
				></p>
			</div>
