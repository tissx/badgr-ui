...
###Added by manprax
1. Edit the   client_id = "SOrfOPHNLouGBNrhY6pPvzxBEL8Ad39qI1KnGOVH"; (line no 62 )   in the  file /edx/app/badgr/badgr-ui/src/app/common/services/session.service.ts.
2. Copy the example file, `environment.prod.ts.example` to `environment.prod.ts` and modify it as needed.Edit it as :
...
export const environment: BadgrEnvironment = {
       production: true,
       config: {
               api: { baseUrl: "REPLACE WITH YOUR BADGE SERVER BASE URL",},
                }
                            }  ;
